...
RserveAov2 <- function(tokens,updateURL,...){
  ...
  RSeval(rookEnv$RserveConnection, quote(suppressPackageStartupMessages(require(BayesFactor.gui, quietly=TRUE))))
...
aovApp <- function(){
  Builder$new(
   Static$new(
      urls = '/www',
      root = system.file('.', package='BayesFactor.gui')
    ),
    URLMap$new(
      '^/data' = function(env){
        req <- Request$new(env)
