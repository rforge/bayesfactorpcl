ttest.Gibbs.AR = function(before,after,iterations=1000,areaNull=c(-.2,.2),treat=NULL,r.scale=1,alphaTheta=1,betaTheta=5,sdMet=.3, progress=TRUE,return.chains=FALSE)
{
	...
	chains = .Call("RgibbsTwoSampleAR", as.numeric(y), N, treat, r.scale, alphaTheta, betaTheta, areaNull[1], areaNull[2], iterations, sdMet, progress, pbFun, new.env(), package="BayesSingleSub")
	...
	if(return.chains)
	{
		...
		return(list(c(logbf.delta=logbf)))
	}
}
...
ttest.MCGQ.AR = function(before,after,iterations=1000,treat=NULL,method="MC",r.scale=1,alphaTheta=1,betaTheta=5)
{
	...
	if(method=="MC")
	{
		logbf = .Call("RMCTwoSampleAR", as.numeric(y), N, treat, r.scale, alphaTheta, betaTheta, iterations, package="BayesSingleSub") - mlike0.gq
	}else if(method=="GQ")
	{
